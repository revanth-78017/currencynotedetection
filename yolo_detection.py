...
class CurrencyNotesDetection:
    ...
    def load_model(self, model_name):
        ...
        # Monkey patch torch.load to force weights_only=False globally for YOLOv5 load
    ...
    def get_text(self,labelCnt):
        ...
        for k,v in labelCnt.items():
            denom = k.replace('Rupees', '')
            note_str = "notes" if v > 1 else "note"
            if v > 1:
                text += f"{v} {denom} rupees {note_str} detected "
            else:
                text += f"{denom} rupees {note_str} detected "
