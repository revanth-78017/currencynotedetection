...
window.onload = () => {
		...
		if (input.files && input.files[0]) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					console.log("English text from server:", englishtext);
					imagebox.attr('src', 'data:image/jpeg;base64,' + image);

					// Set the text immediately into the DOM so it's ready for animateProgress
					$("#detected-text-output").text(englishtext);
	...
	voiceEnglishOutputButton.addEventListener('click', () => {
		...
		let englishVoice = voices.find(v => v.lang.includes('en-IN') || v.lang.includes('en-GB') || v.lang.includes('en-US'));
	...
	voiceHindiOutputButton.addEventListener('click', () => {
		...
		let voices = speechSynthesis.getVoices();
		let hindiVoice = voices.find(v => v.lang.includes('hi-IN') || v.lang.includes('hi'));
...
function progress() {
	...
	function animateProgress() {
		if (count == 100 && per == 360) {
			...
			percent.innerText = "Detection Complete!!"
			percent.style.fontSize = "20px";

			text.style.display = "block";
			// Show the text here immediately without waiting for the timeout!
			$("#detected-text-output").show();
